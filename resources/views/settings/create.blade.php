...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-md overflow-hidden m-4">
    <div class="p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">➕ Tambah Kantor Baru</h2>
        ...
        <form action="{{ route('setting.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none" placeholder="Contoh: Cabang Jakarta">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">Latitude</label>
                    <input type="text" name="latitude" id="lat-input" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
                </div>
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">Longitude</label>
                    <input type="text" name="longitude" id="long-input" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
                </div>
            </div>
            ...
            <button type="button" onclick="getCurrentLocationForOffice()" class="mb-4 w-full bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded transition duration-200">
                📍 Ambil Koordinat Lokasi Saya Saat Ini
            </button>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="radius" value="50" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
            </div>

            <div class="flex justify-between">
                <a href="{{ route('setting.index') }}" class="text-gray-500 hover:text-gray-700 font-bold py-2 px-4">Batal</a>
                <button type="submit" class="bg-blue-900 hover:bg-blue-800 text-white font-bold py-2 px-6 rounded-lg transition duration-200">Simpan Data</button>
            </div>
        </form>
    </div>
</div>
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    statusText.innerText = "Lokasi berhasil didapatkan!";
